...
function setup_mhd_fields_and_base_state(coords, dist, shell, sphere_inner, sphere_outer, φ, θ, r;
    ...
    ê_φ = VectorField(dist, coords, bases=shell.radial_basis)
    ê_φ['g'][0] = 1  # Azimuthal unit vector (φ-component)
    ...
    return (u, p, Θ, B, A, τ_u1, τ_u2, τ_Θ1, τ_Θ2, τ_B1, τ_B2, τ_A1, τ_A2, τ_p,
            T0, B0_phi, U0_phi, ê_r, ê_φ, ê_z, r_vec)
end
...
function solve_critical_rayleigh_number(;
    ...
    fields = setup_mhd_fields_and_base_state(
        coords, dist, shell, sphere_inner, sphere_outer, φ, θ, r,
        B0_amplitude=B0_amplitude, B0_profile=B0_profile, Ri=Ri, Ro=Ro
    )
    u, p, Θ, B, A, τ_u1, τ_u2, τ_Θ1, τ_Θ2, τ_B1, τ_B2, τ_A1, τ_A2, τ_p = fields[1:14]
    T0, B0_phi, U0_phi, ê_r, ê_φ, ê_z, r_vec = fields[15:21]
    ...
    critical_m = argmin(Dict(m => result.Ra_critical for (m, result) in critical_results))
