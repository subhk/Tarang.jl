...
# The FFT-based batched DCT-**II** section that used to sit here (batched R2C/C2R
# plan caches, six 3-D twiddle kernels and `local_dct_along_dim!`) has been
# removed along with the rest of the DCT-II implementation in `dct.jl`. It was
# never reachable: `local_transform_along_dim!` above routes `:chebyshev` axes to
# `local_dct1_along_dim!` (DCT-I), which is the convention this framework's
# Chebyshev basis actually uses.
