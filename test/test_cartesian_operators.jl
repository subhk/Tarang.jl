...
@testset "Operator Matrix Methods" begin
    ...
    @testset "half-dimensional Cartesian gradient" begin
        coords = CartesianCoordinates("x", "y", "z")
        ...
        bases = (xb, zb)  # No basis for y (constant dimension)

        f = ScalarField(dist, "f", bases, Float64)

        x = local_grid(xb, dist, 1.0)
        z = local_grid(zb, dist, 1.0)
        f_grid = sin.(reshape(x, :, 1)) .+ cos.(reshape(z, 1, :))
        f["g"] .= f_grid

        grad_f = evaluate(CartesianGradient(f, coords))

        gy = Array(get_grid_data(grad_f.components[2]))
        gx = Array(get_grid_data(grad_f.components[1]))
        gz = Array(get_grid_data(grad_f.components[3]))
