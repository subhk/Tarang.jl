...
@testset "Plot data extraction" begin
    ...
    for (idx, _) in enumerate(field1.data_g)
        field1.data_g[idx] = idx
    end
    ...
    for (idx, _) in enumerate(field2.data_g)
        field2.data_g[idx] = idx
    end
