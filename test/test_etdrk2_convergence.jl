...
function phi2(z::Float64)
    if abs(z) < 1e-8
        return 0.5 + z/6.0 + z^2/24.0 + z^3/120.0
    else
        return (exp(z) - 1.0 - z) / z^2
    end
end
...
function etdrk2_standard_nonlinear(u_n::Float64, λ::Float64, h::Float64, t_n::Float64)
    """Standard ETDRK2 (Cox-Matthews Eq. 22) for du/dt = λu + u²"""
    ...
    # Stage 1: Predictor  c = exp(hL)u_n + h·φ₁·N(u_n)
    a_n = exp_hL * u_n
    N_u_n = u_n * u_n
    c = a_n + h * phi1_hL * N_u_n

    # Stage 2: Corrector  u_{n+1} = a_n + h·(φ₁-φ₂)·N(u_n) + h·φ₂·N(c)
    N_c = c * c
    u_new = a_n + h * (phi1_hL - phi2_hL) * N_u_n + h * phi2_hL * N_c
