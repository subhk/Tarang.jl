...
@testset "Distributed TransposableField (rank=$rank)" begin
    ...
    # NOTE: 3D TransposableField with 2D mesh is tested separately in GPU tests.
    # The Transpose Counts Computation test was removed because:
    # 1. Creating 3D Domain with 2D mesh triggers PencilFFT planning which is complex to bypass
    # 2. TransposableField is primarily designed for GPU+MPI (not CPU+MPI with PencilArrays)
    # 3. The transpose count computation is tested implicitly in the transpose operation tests below
...
@testset "Async Transpose Operations (rank=$rank)" begin
    ...
    # TransposableField tests require:
    # 1. use_pencil_arrays=false (TransposableField uses its own transpose, not PencilFFTs)
    # 2. ComplexFourier bases (RealFourier/rfft is incompatible with custom transposes)
    # 3. 2D mesh for 3D data (TransposableField auto-creates 2D topology, field allocation must match)
    if nprocs >= 4
        @testset "Async Z to Y transpose" begin
            coords = CartesianCoordinates("x", "y", "z")
            # 3D TransposableField requires 2D mesh matching its topology
            mesh_2d = (2, nprocs ÷ 2)
            dist = Distributor(coords; mesh=mesh_2d, dtype=Float64, architecture=CPU(),
                               use_pencil_arrays=false)

            xbasis = ComplexFourier(coords, "x", 8)
            ybasis = ComplexFourier(coords, "y", 8)
            zbasis = ComplexFourier(coords, "z", 8)
...
@testset "Transpose Operations (rank=$rank)" begin
    ...
    # 2. ComplexFourier bases (RealFourier/rfft is incompatible with custom transposes)
    # 3. 2D mesh for 3D data (TransposableField auto-creates 2D topology, field allocation must match)
    # Skip these tests if we don't have enough processes for a proper 2D mesh
    if nprocs >= 4
