...
@testset "DCT Implementation Correctness" begin
    ...
    # Create test data: f(x) = cos(2πx) on Chebyshev points
    # Chebyshev points: x_j = cos(π(j+0.5)/N) for j=0,...,N-1
    cheb_points = [cos(π * (j + 0.5) / N) for j in 0:N-1]

    # Test function: simple polynomial T_2(x) = 2x² - 1
...
# ============================================================================
# Test 2: Round-trip transform accuracy
# ============================================================================
...
@testset "Round-trip Transform Accuracy" begin
    ...
    println("✓ Round-trip error: $error (should be < 1e-12)")
end

# ============================================================================
# Test 3: 2D Serial Transform
# ============================================================================
...
@testset "2D Serial Chebyshev Transform" begin
    ...
    scale_zero = 1.0 / Nx / 2.0
    scale_pos = 1.0 / Nx

    # Transform along x (axis 1)
    temp = zeros(Nx)
    for j in 1:Ny
        mul!(temp, plan_x, view(data, :, j))
        data[1, j] = temp[1] * scale_zero
        for i in 2:Nx
            data[i, j] = temp[i] * scale_pos
        end
    end

    # Transform along y (axis 2)
    scale_zero_y = 1.0 / Ny / 2.0
    scale_pos_y = 1.0 / Ny
    temp_y = zeros(Ny)
    for i in 1:Nx
        mul!(temp_y, plan_y, view(data, i, :))
        data[i, 1] = temp_y[1] * scale_zero_y
        for j in 2:Ny
            data[i, j] = temp_y[j] * scale_pos_y
        end
    end
    ...
    # Backward x
    for j in 1:Ny
        temp[1] = data[1, j] * scale_bwd_zero
        for i in 2:Nx
            temp[i] = data[i, j] * scale_bwd_pos
        end
        result = plan_x_inv * temp
        data[:, j] = result
    end
...
@testset "Transpose Logic" begin
    ...
    expected_chunk1 = vec(data[1:chunk_x, :])
    actual_chunk1 = send_buf[1:chunk_x*Ny_local]

    # The packing interleaves by j, so we need to compare correctly
    for j in 1:Ny_local
        for i in 1:chunk_x
