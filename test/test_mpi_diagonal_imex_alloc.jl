...
function _alloc_per_step(stepper; N=128, warmup=15, measure=20, dt=1e-3)
    ...
    problem = InitialValueProblem([q]); add_parameters!(problem, nu=0.02); add_equation!(problem, "dt(q) - nu*lap(q) = 0")
...
function _recycler_reuses_stash(stepper; N=16, dt=1e-3)
    ...
    yb = RealFourier(coords["y"]; size=N, bounds=(0.0, 2π))
    domain = Domain(dist, (xb, yb)); q = ScalarField(domain, "q")
    problem = InitialValueProblem([q]); add_parameters!(problem, nu=0.02)
...
function _coupled_sumsq(stepper; N=64, steps=40, dt=1e-3)
    ...
    problem = InitialValueProblem([u, v]); add_parameters!(problem, nu=0.03, c=0.7)
