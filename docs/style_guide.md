...
- Equations: `add_equation!(problem, "∂t(u) = ...")` or `add_bc!` / structured BC helpers.
...
## GPU & Cluster How-To
- Device choice: pass `device="cuda"|"amdgpu"|"metal"|"cpu"` to distributors/fields/solvers/timesteppers. Tarang auto-detects available backends.
- Multi-GPU/cluster: use `Distributor(...; mesh=(Px, Py, Pz))` matching `mpiexec -n Px*Py*Pz`. See `docs/Multi_GPU_Support.md` and `docs/gpu_compatibility_roadmap.md`.
