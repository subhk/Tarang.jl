...
add_equation!(problem, "dt(b) - kappa*lap(b) = -u·grad(b)")
```

### Incorrect Format:
```julia
# ❌ WRONG: Nonlinear terms on LHS
add_equation!(problem, "dt(u) + (u·grad(u)) + grad(p) = nu*lap(u)")
```
...
problem = IVP([u, b, p])
add_equation!(problem, "dt(u) - nu*lap(u) + grad(p) - b*ez = -(u·grad(u))")
add_equation!(problem, "dt(b) - kappa*lap(b) = -u·grad(b)")
add_equation!(problem, "div(u) = 0")
```
