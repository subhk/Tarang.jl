...
- `device`: Computing device ("cpu", "cuda", "rocm", "metal")
...
# GPU distribution
dist = Distributor(coords, mesh=(2, 2), device="cuda")
```
...
dist.device         # String: Computing device
...
# ROCm (AMD)
dist = Distributor(coords, mesh=(2, 2), device="rocm")

# Metal (Apple Silicon)
dist = Distributor(coords, mesh=(2, 2), device="metal")
```
