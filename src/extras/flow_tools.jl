...
qg.θ_bot.data_g .= initial_buoyancy_bottom
qg.θ_top.data_g .= initial_buoyancy_top
...
function extract_surface(field_3d::ScalarField, dim::Symbol, position::Real)
    ...
    coord = Coordinate(field_3d.dist.coordsys, String(dim))
...
function qg_advection_rhs(qg::QGSystem)
    ...
    coord_x = Coordinate(coords_2d, coords_2d.names[1])
    coord_y = Coordinate(coords_2d, coords_2d.names[2])
    ...
    rhs_bot.data_g .= -(qg.u_bot.components[1].data_g .* dθ_dx_bot.data_g .+
                        qg.u_bot.components[2].data_g .* dθ_dy_bot.data_g)
    ...
    rhs_top.data_g .= -(qg.u_top.components[1].data_g .* dθ_dx_top.data_g .+
                        qg.u_top.components[2].data_g .* dθ_dy_top.data_g)
...
function qg_step_euler!(qg::QGSystem, dt::Real)
    ...
    if qg.κ > 0
        ...
        rhs_bot.data_g .+= qg.κ .* frac_lap_bot.data_g
        rhs_top.data_g .+= qg.κ .* frac_lap_top.data_g
    end

    # 5. Euler step
    qg.θ_bot.data_g .+= dt .* rhs_bot.data_g
    qg.θ_top.data_g .+= dt .* rhs_top.data_g
end

function qg_step_rk2!(qg::QGSystem, dt::Real)
    # Save initial state
    θ_bot_0 = copy(qg.θ_bot.data_g)
    θ_top_0 = copy(qg.θ_top.data_g)
    ...
    qg.θ_bot.data_g .= θ_bot_0 .+ dt .* rhs_bot.data_g
    qg.θ_top.data_g .= θ_top_0 .+ dt .* rhs_top.data_g
end
...
function qg_step_rk4!(qg::QGSystem, dt::Real)
    ...
    function compute_rhs()
        ...
        return copy(rhs_bot.data_g), copy(rhs_top.data_g)
    end
    ...
    # k2
    qg.θ_bot.data_g .= θ_bot_0 .+ (dt/2) .* k1_bot
    qg.θ_top.data_g .= θ_top_0 .+ (dt/2) .* k1_top
    k2_bot, k2_top = compute_rhs()

    # k3
    qg.θ_bot.data_g .= θ_bot_0 .+ (dt/2) .* k2_bot
    qg.θ_top.data_g .= θ_top_0 .+ (dt/2) .* k2_top
    k3_bot, k3_top = compute_rhs()

    # k4
    qg.θ_bot.data_g .= θ_bot_0 .+ dt .* k3_bot
    qg.θ_top.data_g .= θ_top_0 .+ dt .* k3_top
    k4_bot, k4_top = compute_rhs()

    # Final update
    qg.θ_bot.data_g .= θ_bot_0 .+ (dt/6) .* (k1_bot .+ 2 .* k2_bot .+ 2 .* k3_bot .+ k4_bot)
    qg.θ_top.data_g .= θ_top_0 .+ (dt/6) .* (k1_top .+ 2 .* k2_top .+ 2 .* k3_top .+ k4_top)
end
...
# Set initial condition
bad.fields["surface"].data_g .= initial_condition
...
bad.velocities["concentration"].components[1].data_g .= u_x
bad.velocities["concentration"].components[2].data_g .= u_y
```
...
function bad_compute_velocity!(bad::BoundaryAdvectionDiffusion)
    ...
    elseif isa(bad.velocity_source, SelfDerivedVelocity)
        ...
        for bspec in bad.boundary_specs
            ...
            bad.velocities[bspec.name].components[1].data_g .= vel.components[1].data_g
            bad.velocities[bspec.name].components[2].data_g .= vel.components[2].data_g
        end
...
function bad_compute_rhs!(bad::BoundaryAdvectionDiffusion, boundary_name::String)
    ...
    # Get coordinates
    coord_x = Coordinate(bad.coords, bad.coords.names[1])
    coord_y = Coordinate(bad.coords, bad.coords.names[2])
    ...
    # Advection term: -u·∇c
    rhs.data_g .= -(velocity.components[1].data_g .* dfdx.data_g .+
                    velocity.components[2].data_g .* dfdy.data_g)
...
function compute_diffusion_term(bad::BoundaryAdvectionDiffusion, field::ScalarField)
    ...
    elseif bad.diffusion.type == :laplacian
        ...
        result.data_g .= κ .* lap_field.data_g
    ...
    elseif bad.diffusion.type == :fractional
        ...
        result.data_g .= -κ .* frac_lap.data_g
...
function bad_step_euler!(bad::BoundaryAdvectionDiffusion, dt::Real)
    for bspec in bad.boundary_specs
        ...
        bad.fields[bspec.name].data_g .+= dt .* rhs.data_g
    end
end
...
function bad_step_rk2!(bad::BoundaryAdvectionDiffusion, dt::Real)
    ...
    for bspec in bad.boundary_specs
        ensure_layout!(bad.fields[bspec.name], :g)
        saved_states[bspec.name] = copy(bad.fields[bspec.name].data_g)
    end
    ...
    for bspec in bad.boundary_specs
        rhs = bad_compute_rhs!(bad, bspec.name)
        bad.fields[bspec.name].data_g .= saved_states[bspec.name] .+ dt .* rhs.data_g
    end
end
...
function bad_step_rk4!(bad::BoundaryAdvectionDiffusion, dt::Real)
    ...
    for bspec in bad.boundary_specs
        rhs = bad_compute_rhs!(bad, bspec.name)
        k1[bspec.name] = copy(rhs.data_g)
    end

    # k2
    for bspec in bad.boundary_specs
        bad.fields[bspec.name].data_g .= saved_states[bspec.name] .+ (dt/2) .* k1[bspec.name]
    end
    if bad.interior_problem !== nothing
        bad_solve_interior!(bad)
    end
    ...
    for bspec in bad.boundary_specs
        rhs = bad_compute_rhs!(bad, bspec.name)
        k2[bspec.name] = copy(rhs.data_g)
    end

    # k3
    for bspec in bad.boundary_specs
        bad.fields[bspec.name].data_g .= saved_states[bspec.name] .+ (dt/2) .* k2[bspec.name]
    end
    if bad.interior_problem !== nothing
        bad_solve_interior!(bad)
    end
    ...
    for bspec in bad.boundary_specs
        rhs = bad_compute_rhs!(bad, bspec.name)
        k3[bspec.name] = copy(rhs.data_g)
    end

    # k4
    for bspec in bad.boundary_specs
        bad.fields[bspec.name].data_g .= saved_states[bspec.name] .+ dt .* k3[bspec.name]
    end
    if bad.interior_problem !== nothing
        bad_solve_interior!(bad)
    end
    ...
    for bspec in bad.boundary_specs
        rhs = bad_compute_rhs!(bad, bspec.name)
        k4[bspec.name] = copy(rhs.data_g)
    end

    # Final update
    for bspec in bad.boundary_specs
        bad.fields[bspec.name].data_g .= saved_states[bspec.name] .+
...
function bad_step_ssprk3!(bad::BoundaryAdvectionDiffusion, dt::Real)
    ...
    for bspec in bad.boundary_specs
        stage1_states[bspec.name] = copy(bad.fields[bspec.name].data_g)
        rhs = bad_compute_rhs!(bad, bspec.name)
        bad.fields[bspec.name].data_g .= 0.75 .* saved_states[bspec.name] .+
            0.25 .* (stage1_states[bspec.name] .+ dt .* rhs.data_g)
    end
    ...
    for bspec in bad.boundary_specs
        stage2_state = copy(bad.fields[bspec.name].data_g)
        rhs = bad_compute_rhs!(bad, bspec.name)
        bad.fields[bspec.name].data_g .= (1/3) .* saved_states[bspec.name] .+
            (2/3) .* (stage2_state .+ dt .* rhs.data_g)
    end
end
...
function bad_energy(bad::BoundaryAdvectionDiffusion)
    ...
    for bspec in bad.boundary_specs
        field = bad.fields[bspec.name]
        ensure_layout!(field, :g)
        energy = sum(abs2.(field.data_g)) / length(field.data_g)
        total_energy += energy
    end
...
function bad_enstrophy(bad::BoundaryAdvectionDiffusion, boundary_name::String)
    ...
    enstrophy = sum(abs2.(dfdx.data_g) .+ abs2.(dfdy.data_g)) / length(dfdx.data_g)
...
function bad_max_velocity(bad::BoundaryAdvectionDiffusion)
    ...
    for bspec in bad.boundary_specs
        ...
        vel_mag = sqrt.(vel.components[1].data_g.^2 .+ vel.components[2].data_g.^2)
...
function qg_energy(qg::QGSystem)
    ...
    coord_x = Coordinate(coords, coords.names[1])
    coord_y = Coordinate(coords, coords.names[2])
    coord_z = Coordinate(coords, coords.names[3])
