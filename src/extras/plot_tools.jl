...
function vector_magnitude(field::VectorField; layout::Symbol=:g)
    ...
    # Calculate |u|² using the mag_field's own buffer (same architecture as components)
    layout_str = string(layout)
    mag_data = mag_field[layout_str]
    mag_data .= 0

    for component in field.components
        ensure_layout!(component, layout)
        mag_data .+= abs2.(component[layout_str])
    end

    mag_data .= sqrt.(mag_data)
...
export save_field_netcdf, save_field_csv
