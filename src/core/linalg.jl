...
# Note: stats are updated from threaded code (e.g., threaded_block_matmat!).
# The lock protects against concurrent increments; reads for diagnostics
# are racy but acceptable for profiling counters.
mutable struct LinalgPerformanceStats
    ...
    function LinalgPerformanceStats()
        new(0, 0.0, 0, 0.0, 0, 0, 0, ReentrantLock())
    end
end
