...
# Device management for GPU compatibility
...
struct DeviceConfig
    ...
    function DeviceConfig(device_type::DeviceType=CPU_DEVICE, device_id::Int=0)
        new(device_type, device_id)
    end
end
...
function device_array(A::AbstractArray, config::DeviceConfig)
...
function device_zeros(T::Type, dims, config::DeviceConfig)
...
function device_identity(T::Type, n::Int, config::DeviceConfig)
