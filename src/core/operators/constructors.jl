...
Algebraic trace reduction for first-order formulation.

Instead of evaluating tensor expressions, applies mathematical identities
at construction time to convert to scalar/vector ops:
...
Works with BOTH type hierarchies (Operator: AddOperator, MultiplyOperator
and Future: Add, Multiply) via structural dispatch on expression shape.
"""
function trace(expr)
    # ── Concrete TensorField: standard trace ────────────────────
    isa(expr, TensorField) && return multiclass_new(Trace, expr)

    # ── trace(grad(v)) = div(v) ─────────────────────────────────
    isa(expr, Gradient) && return divergence(expr.operand)

    # ── Linearity: distribute over ±  ───────────────────────────
    args = _get_sum_args(expr)
    if args !== nothing
        return reduce(+, trace.(args))
    end
    neg_inner = _get_negated(expr)
    if neg_inner !== nothing
        return -(trace(neg_inner))
    end

    # ── trace(c * A) = c * trace(A)  or  trace(v⊗w) = v⋅w ─────
    factors = _get_product_factors(expr)
    if factors !== nothing
        scalars = filter(_is_scalar_like, factors)
        tensors = filter(!_is_scalar_like, factors)
        if !isempty(scalars) && !isempty(tensors)
            s = reduce(*, scalars)
            t = length(tensors) == 1 ? trace(tensors[1]) : _dot_components(tensors...)
            return s * t
        elseif length(tensors) == 2
            return _dot_components(tensors[1], tensors[2])
        end
    end

    # ── Fallback: wrap in Trace node for runtime evaluation ─────
    return Trace(expr)
end

# ── Structural accessors (work with Operator AND Future types) ──

"""Get addends from Add or AddOperator, or nothing."""
function _get_sum_args(expr)
    isa(expr, AddOperator) && return Any[expr.left, expr.right]
    isa(expr, Add)         && return collect(Any, future_args(expr))
    isa(expr, SubtractOperator) && return Any[expr.left, NegateOperator(expr.right)]
    isa(expr, Subtract)    && let a = future_args(expr)
        return length(a) >= 2 ? Any[a[1], NegateOperator(a[2])] : nothing
    end
    return nothing
end

"""Get inner operand from Negate/NegateOperator, or nothing."""
_get_negated(expr) = isa(expr, NegateOperator) ? expr.operand :
                     isa(expr, Negate) ? future_args(expr)[1] : nothing

"""Get factors from Multiply/MultiplyOperator, or nothing."""
function _get_product_factors(expr)
    isa(expr, MultiplyOperator) && return Any[expr.left, expr.right]
    isa(expr, Multiply)         && return collect(Any, future_args(expr))
    return nothing
end

_is_scalar_like(x) = isa(x, Number) || isa(x, ScalarField) ||
                     isa(x, ConstantOperator) || isa(x, ZeroOperator)

"""Dot product via component expansion: Σᵢ aᵢ * bᵢ."""
function _dot_components(a, b)
    ac = _vector_components(a)
    bc = _vector_components(b)
    if ac !== nothing && bc !== nothing
        n = min(length(ac), length(bc))
        return reduce(+, (ac[i] * bc[i] for i in 1:n))
    end
    return Trace(a * b)  # can't decompose → defer
end

"""
    _vector_components(expr)

Extract scalar components from any vector-like expression, handling:
- VectorField directly
- Single-operand wrappers: Negate, Lift, Gradient, etc.
- Future wrappers: Negate(VectorField), Multiply(scalar, VectorField)
"""
function _vector_components(expr)
    # Direct VectorField
    isa(expr, VectorField) && return collect(expr.components)

    # Single-operand Operator wrappers
    if hasfield(typeof(expr), :operand)
        inner = _vector_components(expr.operand)
    ...
    if isa(expr, Future)
        ...
        if isa(expr, Negate) && length(args) >= 1
            inner = _vector_components(args[1])
            return inner !== nothing ? [-c for c in inner] : nothing
        end
        if isa(expr, Multiply) && length(args) == 2
            # scalar * vector → scale each component
            if _is_scalar_like(args[1])
                inner = _vector_components(args[2])
                return inner !== nothing ? [args[1] * c for c in inner] : nothing
            elseif _is_scalar_like(args[2])
                inner = _vector_components(args[1])
                return inner !== nothing ? [c * args[2] for c in inner] : nothing
            end
        end
        # Single-arg Future wrapping a vector
        if length(args) == 1
            return _vector_components(args[1])
        end
    end
...
Following spectral methods pattern - splits CartesianCoordinates into individual coords.
"""
...
Following spectral methods pattern - splits DirectProduct into coordinate systems.
"""
...
Following spectral methods pattern - splits CartesianCoordinates and recurses.
"""
...
Following spectral methods pattern - splits DirectProduct and recurses.
"""
...
Note: only allows n < 0. We allow positive n for flexibility,
...
Matches spectral syntax: `lift(tau, -1)`.
