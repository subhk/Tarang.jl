...
function dispatch_check(::Type{Trace}, args::Tuple, kwargs::NamedTuple)
    # Accept TensorField directly, or deferred expression trees (Operator, Future)
    # that will evaluate to a tensor at runtime (e.g. trace(grad_u) in first-order form).
    return true
end
