...
function _unpack_gathered_y!(dest::AbstractArray{T,2}, buffer::AbstractVector{T},
    ...
    # GPU-compatible: use views + broadcasting (no CPU round-trip)
    local_nx = z_shape[1]
    total_ny = y_shape[2]
    buf_offset = 0
    for rank in 0:(nranks-1)
        ...
        rank_ny = length(y_range)
        y_start = first(y_range)
        block_size = local_nx * rank_ny
        block = reshape(view(buffer, buf_offset+1 : buf_offset+block_size), local_nx, rank_ny)
        dest[:, y_start : y_start+rank_ny-1] .= block
...
function _pack_scatter_y!(buffer::AbstractVector{T}, src::AbstractArray{T,2},
    ...
    rank_ny = length(y_range)
    block_size = local_nx * rank_ny
    block = reshape(view(buffer, 1:block_size), local_nx, rank_ny)
