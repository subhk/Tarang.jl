...
function distributed_forward_transform!(tf::TransposableField{F,T,N};
    ...
    if N >= 3
        ...
        # TODO(perf): These two extra all-to-all transposes double the communication
        # cost (4 transposes per direction instead of 2). Fix by storing spectral data
        # in XLocal layout directly, which requires changing field["c"] allocation
        # conventions and updating backward_transform to start from XLocal.
        transpose_x_to_y!(tf)
        transpose_y_to_z!(tf)
