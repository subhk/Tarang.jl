...
mutable struct CachedClass
    ...
    function CachedClass()
        new(Dict{Any, WeakRef}(), ReentrantLock())
    end
end
...
const _cached_class_instances_lock = ReentrantLock()

# ============================================================================
# LRU Cache (must be defined before function_cache which uses it)
# ============================================================================
...
# Simple caching for function results with bounded LRU eviction
const _FUNCTION_CACHE_MAX_SIZE = 4096
const function_cache = LRUCache{Tuple{Function, Any}, Any}(_FUNCTION_CACHE_MAX_SIZE)

function cached_call(func::Function, args...)
    key = (func, args)
    return get!(function_cache, key) do
        func(args...)
    end
end
        ...
        for (i, buf) in enumerate(wm.real_buffers)
            if !wm.real_in_use[i]
                ...
                if buf_size == target_size || (buf_size >= target_size && buf_size <= target_size * wm.size_tolerance)
                    ...
                    wm.stats["reuses"] += 1
                    # Reshape/view if shape differs but total size is compatible
                    if size(buf) == shape
                        return buf
                    else
...
function release_workspace!(wm::WorkspaceManager, buf::AbstractArray)
    # Try to find the underlying Array through view/reshape wrappers
    underlying = buf
    while underlying isa SubArray || underlying isa Base.ReshapedArray
        underlying = parent(underlying)
    end
    # If we unwrapped to a managed type, release it
    if underlying isa Array{Float64} || underlying isa Array{ComplexF64}
        release_workspace!(wm, underlying)
        return nothing
    end
    # Truly unmanaged types — nothing to do
    return nothing
end
