...
function split_equation(equation::AbstractString)
    ...
    while idx <= lastindex(equation)
        ...
        elseif character == '=' && isempty(stack)
            ...
            prev_idx = prevind(equation, idx)
            next_idx = nextind(equation, idx)
            prev_char = idx > 1 ? equation[prev_idx] : '\0'
            next_char = next_idx <= lastindex(equation) ? equation[next_idx] : '\0'
            if next_char != '=' && prev_char != '!' && prev_char != '>' && prev_char != '<' && prev_char != '='
