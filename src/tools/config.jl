...
function getstring(section::ConfigSection, key::String; default::Union{String, Nothing}=nothing)
    ...
    if value === nothing
        ...
        throw(KeyError("Key '$key' in section '$(section.name)' has value `nothing` and no default was provided"))
    end
    return string(value)
end
    ...
    @lock CONFIG_LOCK begin
        try
            ...
            if merge
                ...
                # Start from defaults and overlay file values so that
                # keys not present in the file retain their defaults
                fresh = _create_default_config()
                merge_config!(fresh, file_config)
                config.data = fresh
            end
...
function apply_env_overrides!(config::Config)
    @lock CONFIG_LOCK begin
        ...
        if haskey(ENV, "JULIA_NUM_THREADS")
            val = ENV["JULIA_NUM_THREADS"]
            if lowercase(val) == "auto"
                # "auto" is valid in Julia 1.5+ - use actual thread count
                config.data["threading"]["JULIA_THREADS"] = Threads.nthreads()
            else
                parsed = tryparse(Int, val)
                if parsed === nothing
                    @warn "Invalid JULIA_NUM_THREADS: $val, ignoring"
                else
