...
Create a chunked random number generator.

!!! warning "MPI usage"
    When `seed` is explicitly provided, all MPI ranks produce identical sequences.
    For spatially varying noise across ranks, combine the seed with the rank:
    `chunked_rng(UInt64(seed) + UInt64(MPI.Comm_rank(MPI.COMM_WORLD)), ...)`
"""
