...
"""Solve upper triangular sparse system using back-substitution."""
function solve_upper_sparse(A::SparseMatrixCSC, b::AbstractVector)
    return UpperTriangular(A) \ b
end

"""Solve lower triangular sparse system using forward-substitution."""
...
function is_contiguous(arr::AbstractArray)
    ...
    # SubArrays: use Julia's built-in stride check rather than recursing to parent,
    # since a strided view of a contiguous array is NOT contiguous.
    if arr isa SubArray
        return Base.iscontiguous(arr)
    end
    # ReshapedArrays of contiguous parents are contiguous
    if arr isa Base.ReshapedArray
        return is_contiguous(parent(arr))
    end
    return false
end
...
function safe_cast(arr::AbstractArray{T}, target_type::Type{U}) where {T, U}
    ...
    if T <: AbstractFloat && U <: Integer
        @warn "Converting from $T to $U using truncation" maxlog=1
        return U.(trunc.(arr))
    end

    # Check for complex-to-real conversion
    if T <: Complex && !(U <: Complex)
        @warn "Converting from $T to $U, imaginary parts will be discarded" maxlog=1
